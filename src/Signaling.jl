...
#This file create the numerical simulations and functions used to generate figure 6
...
"""
    We will now create the functions needed to plot the graphs for figure 6. 
...
function errmktclear(pi::Any,theta::Any)
    ...
    y= quadgk(g,theta,lambda) ## this takes the integral of g

      return y[1]-1 ## the quadgk function returns a vector where the first entry corresponds
                    ##the value of the integral. hence the y[1]
end
...
profits[2,:] = ((lambda - thetasdev[2,:])*qL + pistar[2]*(1-lambda)*qH)./ (lambda - thetasdev[2,:] + pistar[2]*(1-lambda)) .-wstar
 ...
 for j = 1:N
    ...
    wD[1,j] = qL + (qH - qL) * (1-cH/cL * a[1]) ## multiply with only the first output of the integral
